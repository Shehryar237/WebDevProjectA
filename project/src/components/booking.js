...
const Booking = () => {
  ...
  const updatedBookings = bookings.map(booking => {
    if (booking.id === selectedBooking.id) {
      return { ...booking, available_seats: booking.available_seats - 1 };
    }
    return booking;
  });
  setBookings(updatedBookings);
  setShowPopup(false);
  window.alert('Booking successful!'); // Show alert after form submission and popup close
};
  return (
    <div>
      <Navbar/>
          ...
          <div className='bg-white p-8 w-full md:max-w-md'>
            ...
            <form onSubmit={handleSubmit}>
              <div className='mb-4'>
                ...
                <input type='text' id='name' name='name' value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className='w-full px-3 py-2 border rounded' />
              </div>
              <div className='mb-4'>
                <label htmlFor='email' className='block text-gray-700 font-bold mb-2'>Email:</label>
                <input type='email' id='email' name='email' value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} className='w-full px-3 py-2 border rounded' />
              </div>
              <div className='mb-4'>
                <label htmlFor='contact' className='block text-gray-700 font-bold mb-2'>Contact Number:</label>
                <input type='text' id='contact' name='contact' value={formData.contact} onChange={(e) => setFormData({ ...formData, contact: e.target.value })} className='w-full px-3 py-2 border rounded' />
              </div>
              <button type='submit' className='bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded'>Submit</button>
              <button type='button' onClick={() => setShowPopup(false)} className='bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded ml-2'>Cancel</button>
            </form>
          </div>
        </div>
      )}
